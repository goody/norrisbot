...
NorrisBot.prototype._replyWithRandomJoke = function (originalMessage) {
    ...
    //     }
            ...
            request('http://api.icndb.com/jokes/' + (250) + '?escape=javascript', function (err, response, body) {
                // handle possible request errors by stopping the whole process
                if (err || response.statusCode !== 200) {
                    console.log(250, err, response.statusCode);
        ...
        self.postMessageToChannel(channel.name, body.joke, {as_user: true});
                } catch (ex) {
                    return ex;
                }
